...
class Showo(ModelMixin, ConfigMixin):
    ...
    def resize_token_embeddings(self, new_vocab_size):
        old_vocab_size, embedding_dim = self.showo.model.embed_tokens.weight.shape
        
        # Create a new embedding layer
        new_embedding = nn.Embedding(new_vocab_size, embedding_dim)
        
        # Initialize the new embeddings with the old weights
        with torch.no_grad():
            if new_vocab_size <= old_vocab_size:
                # Truncate the embeddings if the new vocab size is smaller
                new_embedding.weight[:new_vocab_size] = self.showo.model.embed_tokens.weight[:new_vocab_size]
            else:
                # Copy old weights and initialize new embeddings
                new_embedding.weight[:old_vocab_size] = self.showo.model.embed_tokens.weight
                new_embedding.weight[old_vocab_size:] = torch.nn.init.normal_(new_embedding.weight[old_vocab_size:], mean=0.0, std=0.02)
        
        # Replace the embedding layer in the model
        self.showo.model.embed_tokens = new_embedding
        
        # Resize the output layer (lm_head) to match the new vocab size
        old_out_features, in_features = self.showo.lm_head.weight.shape
        new_lm_head = nn.Linear(in_features, new_vocab_size, bias=False)
        
        with torch.no_grad():
            if new_vocab_size <= old_out_features:
                new_lm_head.weight[:new_vocab_size] = self.showo.lm_head.weight[:new_vocab_size]
            else:
                new_lm_head.weight[:old_out_features] = self.showo.lm_head.weight
                new_lm_head.weight[old_out_features:] = torch.nn.init.normal_(new_lm_head.weight[old_out_features:], mean=0.0, std=0.02)
        
        # Replace the lm_head in the model
        self.showo.lm_head = new_lm_head
