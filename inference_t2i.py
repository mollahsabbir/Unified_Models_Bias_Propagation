...
if __name__ == '__main__':
    ...
    if config.mode == 'inpainting':
        ...
        for step in tqdm(range(0, len(prompts_df), config.training.batch_size)):
            prompts_row = prompts_df.iloc[step:step + config.training.batch_size]
            prompts = prompts_df["Prompt"][step:step + config.training.batch_size].tolist()
            ...
            images = images.permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)
            pil_images = [Image.fromarray(image) for image in images]
            model_checkpoint = config.model.showo.pretrained_model_path.split("/")[-2].split("-")[-1]
            os.makedirs(f"generated_images_{model_checkpoint}", exist_ok=True)
            import time
            # Save images with timestamp
            for i, img in enumerate(pil_images):
                timestamp = time.strftime("%Y%m%d-%H%M%S")  # Get current timestamp
                filename = f"generated_images_{model_checkpoint}/image_{timestamp}_prompt_{prompts_row.iloc[i]['Object']}_{prompts_row.iloc[i]['Type']}.png"
                img.save(filename)
