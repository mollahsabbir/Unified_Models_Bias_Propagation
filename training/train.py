...
from training.llava_pretrain_t2i_dataset import LLaVAPretrainT2I
...
def main():
    ...
    if config.dataset.gen_type == "t2i":
        ...
        num_train_epochs = math.ceil(config.training.max_train_steps / num_update_steps_per_epoch)
    elif config.dataset.gen_type == "llava_pretrained":
        dataset_llava_pretrained = LLaVAPretrainT2I(
            "/home/sa162103/workspace/datasets/LLaVA_Pretrained/images/", split="train", image_size=256, max_number_of_items=364100
            # "/home/sa162103/workspace/datasets/LLaVA-Pretrained/", split="train", image_size=256, max_number_of_items=364100
        )

        print('process index : ',
              accelerator.process_index, ', ', accelerator.num_processes,
              "Length: ", len(dataset_llava_pretrained))

        if accelerator.num_processes > 1:
            sampler = DistributedSampler(dataset_llava_pretrained,
                                         num_replicas=accelerator.num_processes,
                                         rank=accelerator.process_index,
                                         shuffle=True,
                                         )
        ...
        train_dataloader_t2i = DataLoader(dataset_llava_pretrained, batch_size=config.training.batch_size_t2i,
                                          sampler=sampler,
                                          shuffle=shuffle, num_workers=dataset_config.num_workers)
        num_update_steps_per_epoch = math.ceil(len(dataset_llava_pretrained) / total_batch_size_t2i)
        num_train_epochs = math.ceil(config.training.max_train_steps / num_update_steps_per_epoch)
    else:
